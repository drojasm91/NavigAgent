...
// Weekly cron. Reads like signals per user per snipper, updates prompt_config.
...
export const personalizationJob = schedules.task({
  id: 'snipper-personalization',
  // Every Sunday at 03:00 UTC
  cron: '0 3 * * 0',
  run: async () => {
    const supabase = createServiceClient()

    // Get all users with at least one snipper subscription
